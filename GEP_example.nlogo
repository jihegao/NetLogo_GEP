  ...
  foreach gp_syntaxlist [ [?]->
    if not any? turtles with [label = first ?][
      create-genes 1 [
        ...
        set odds ifelse-value (is-string? last ?)[ read-from-string last ?][last ?]
...
to gp-initialize-codeturtle [gene_lists]
  ifelse gene_lists = [0]
  [
    set my_code (gp-weighted-random-genes initial-DNA-length)
  ]
  [
    set my_code (gp-cross-DNAs gene_lists)
  ]
  carefully [
...
to-report gp-cross-DNAs [DNA_lists]
